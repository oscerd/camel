 ...
 * <p/>
 * Since BouncyCastle 1.85 the pre-standardization {@code DILITHIUM}, {@code SPHINCSPLUS} and {@code KYBER} constants
 * are backed by the standardized ML-DSA, SLH-DSA and ML-KEM algorithms, so they take the corresponding standardized
 * parameter sets (for example {@code ML-DSA-65} for {@code DILITHIUM}).
 */
public final class PQCParameterSpecResolver {
    ...
    private static AlgorithmParameterSpec doResolve(String algorithm, String parameterSpec) {
        switch (algorithm) {
            // Signature algorithms. Since BouncyCastle 1.85 the pre-standardization DILITHIUM and SPHINCSPLUS enum
            // constants are backed by the standardized ML-DSA and SLH-DSA algorithms, so they take those parameter sets
            case "MLDSA":
            case "DILITHIUM":
            ...
            // Key encapsulation algorithms. Likewise KYBER is backed by the standardized ML-KEM algorithm
            case "MLKEM":
            case "KYBER":
